...
dates <- seq(as.Date("2022-05-01"), as.Date("2022-05-30"), by = "days")
  ...
  image_write(
    image_composite(
      image_composite(
        ...
        gravity = "northwest",
        compose_args = ""),
      image_read("https://raw.githubusercontent.com/kaputkin/hit_distance_bot/main/assets/circle_border.png"),
      operator = "atop",
      offset = "+0+0",
      gravity = "northwest",
      compose_args = ""
...
g <- ggplot(df, aes(x = reorder(player_name, total), y = total)) + 
  ...
  geom_text(
    ...
    size = 3.8)+
  #header text
  #ggtitle(paste0("Who's Hit the Furthest?"),
  #       subtitle="As of June 9th, 2023")+
  #labs(title = "Who's Hit the Furthest?",
   #    subtitle= "<span style='font-size:11pt','color:#0072B2;'>As of June 9th, 2023</span>")+
  labs(
    title = paste0("<span style='font-size:18pt;font-family:Calibri;'>**Who's Hit the Furthest?**","<br/>","<span style='font-size:11pt;font-style:bold;'>as of ",month," ",day,", ",year,"      ","<span style='font-size:9pt;color:",handle_color,";'>@howfartheball</span></span>")
      )+
  ...
  theme(
    ...
    #add axis line
    axis.line.x = element_line(size = .7, colour = axis_color, linetype=1),
    # Remove the title for both axes
    axis.title = element_blank(),
    # Remove labels from the vertical axis
    axis.text.y = element_blank(),
    # But customize labels for the horizontal axis
    axis.text.x = element_text(family = "bevan", size = 10, face="bold", color = axis_color, margin = margin(t= 8)),
    #title
...
ggsave("plot.png", g, width = 8*.75, height = 8.58*.75, dpi = 300)
